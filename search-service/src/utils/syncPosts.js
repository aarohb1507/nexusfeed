...
const syncPostsFromPostService = async () => {
  ...
  try {
    ...
    while (true) {
      ...
      // Optimize: Check all postIds in batch with single query (instead of N queries)
      const postIds = posts.map(p => p._id.toString());
      const existingPostIds = await Search.find(
        { postId: { $in: postIds } }
      ).distinct('postId');
      ...
      for (const post of posts) {
        // Check if already indexed using Set (O(1) lookup)
        if (!existingSet.has(post._id.toString())) {
          searchDocs.push({
